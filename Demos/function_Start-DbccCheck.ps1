function Test-DbaLastBackup {
    ...
    process {
        $dbatoolsmod = Import-Module dbatools -PassThru
        ...
        foreach ($db in $InputObject) {
            ...
            if (-not (& $dbatoolsmod {Test-Bound -ParameterName Destination})) {
            ...
            try {
                $global:destserver = Connect-DbaInstance -SqlInstance $Destination -SqlCredential $DestinationSqlCredential
            } catch {
                & $dbatoolsmod { Stop-Function -Message "Failure" -Category ConnectionError -ErrorRecord $_ -Target $Destination -Continue}
            }

            if ($destserver.VersionMajor -lt $sourceserver.VersionMajor) {
                & $dbatoolsmod { Stop-Function -Message "$Destination is a lower version than $instance. Backups would be incompatible." -Continue}
            }
            ...
            if ($CopyPath) {
                ...
                if (-not $testpath) {
                    & $dbatoolsmod { Stop-Function -Message "$destserver cannot access $CopyPath." -Continue}
                }
            } else {
            ...
            if ($instance -ne $destination -and -not $CopyFile) {
                ...
                if ($BackupFolder) {
                    if ($BackupFolder.StartsWith("\\") -eq $false -and $sourcerealname -ne $destrealname) {
                        & $dbatoolsmod { Stop-Function -Message "Backup folder must be a network share if the source and destination servers are not the same." -Continue}
                    }
                }
            }
            ...
            if ($datadirectory) {
                if (-not (Test-DbaPath -SqlInstance $destserver -Path $datadirectory)) {
                    ...
                    & $dbatoolsmod { Stop-Function -Message "Can't access $datadirectory Please check if $serviceAccount has permissions." -Continue}
                }
            } else {
                $datadirectory = & $dbatoolsmod {Get-SqlDefaultPaths -SqlInstance $destserver -FileType mdf}
            }

            if ($logdirectory) {
                if (-not (Test-DbaPath -SqlInstance $destserver -Path $logdirectory)) {
                    $serviceAccount = $destserver.ServiceAccount
                    & $dbatoolsmod { Stop-Function -Message "$Destination can't access its local directory $logdirectory. Please check if $serviceAccount has permissions." -Continue}
                }
            } else {
                $logdirectory = & $dbatoolsmod {Get-SqlDefaultPaths -SqlInstance $destserver -FileType ldf}
            }

            if ((& $dbatoolsmod {Test-Bound -ParameterName AzureCredential}) -and (& $dbatoolsmod {Test-Bound -ParameterName CopyFile})) {
                & $dbatoolsmod { Stop-Function -Message "Cannot use copyfile with Azure backups, set to false." -continue}
                $CopyFile = $false
            }

            & $dbatoolsmod {Write-Message -Level Verbose -Message "Getting recent backup history for $($db.Name) on $instance."}

            if (& $dbatoolsmod {Test-Bound "IgnoreLogBackup"}) {
                & $dbatoolsmod {Write-Message -Level Verbose -Message "Skipping Log backups as requested."}
                $lastbackup = @()
                ...
                if (-not (& $dbatoolsmod {Test-Bound "IgnoreDiffBackup"})) {
            ...
            if (-not $lastbackup) {
                & $dbatoolsmod {Write-Message -Level Verbose -Message "No backups exist for this database."}
            ...
            if ($CopyFile) {
                try {
                    & $dbatoolsmod {Write-Message -Level Verbose -Message "Gathering information for file copy."}
                    $removearray = @()

                    foreach ($backup in $lastbackup) {
                        foreach ($file in $backup.Path) {
                            $filename = Split-Path -Path $file -Leaf
                            & $dbatoolsmod {Write-Message -Level Verbose -Message "Processing $filename."}
                            ...
                            & $dbatoolsmod {Write-Message -Level Verbose -Message "Destination directory is $destdirectory."}
                            & $dbatoolsmod {Write-Message -Level Verbose -Message "Destination filename is $remotedestfile."}

                            try {
                                & $dbatoolsmod {Write-Message -Level Verbose -Message "Copying $sourcefile to $remotedestfile."}
                    ...
                    & $dbatoolsmod {Write-Message -Level Warning -Message "Failed to copy backups for $dbName on $instance to $destdirectory - $_."}
                    $copysuccess = $false
                }
            }
            if (-not $copysuccess) {
                & $dbatoolsmod {Write-Message -Level Verbose -Message "Failed to copy backups."}
                $lastbackup = @{
                ...
                & $dbatoolsmod {Write-Message -Level Verbose -Message "No full backup returned from lastbackup."}
                $lastbackup = @{
                    Path = "Not found"
                }
                $fileexists = $false
                ...
                & $dbatoolsmod {Write-Message -Level Verbose -Message "Path not UNC and source does not match destination. Use -CopyFile to move the backup file."}
                ...
                & $dbatoolsmod {Write-Message -Level Verbose -Message "SQL Server cannot find backup."}
                $fileexists = $false
                $success = $restoreresult = $dbccresult = "Skipped"
            }
            if ($restoreresult -ne "Skipped" -or $lastbackup[0].Path -like 'http*') {
                & $dbatoolsmod {Write-Message -Level Verbose -Message "Looking good."}
                ...
                if ($destdb) {
                    & $dbatoolsmod { Stop-Function -Message "$dbName already exists on $destination - skipping." -Continue}
                }

                if ($Pscmdlet.ShouldProcess($destination, "Restoring $ogdbname as $dbName.")) {
                    & $dbatoolsmod {Write-Message -Level Verbose -Message "Performing restore."}
                    ...
                    try {
                        ...
                        if (& $dbatoolsmod {Test-Bound "MaxTransferSize"}) {
                            $restoreSplat.Add('MaxTransferSize', $MaxTransferSize)
                        }
                        if (& $dbatoolsmod {Test-Bound "BufferCount"}) {
                            $restoreSplat.Add('BufferCount', $BufferCount)
                        }
                        if (& $dbatoolsmod {Test-Bound "FileStreamDirectory"}) {
                ...
                if (-not $NoCheck -and -not $VerifyOnly) {
                    ...
                    if ($ogdbname -eq "master") {
                        ...
                        if ($success -eq "Success") {
                            & $dbatoolsmod {Write-Message -Level Verbose -Message "Starting DBCC."}
                ...
                if (-not $NoDrop -and $null -ne $destserver.databases[$dbName]) {
                    if ($PSCmdlet.ShouldProcess($dbName, "Dropping Database $dbName on $destination")) {
                        & $dbatoolsmod {Write-Message -Level Verbose -Message "Dropping database."}
                        ...
                        try {
                            ...
                            & $dbatoolsmod {Write-Message -Level Verbose -Message "Dropped $dbName Database on $destination."}
                        } catch {
                            $destserver.Databases.Refresh()
                            if ($destserver.databases[$dbName]) {
                                & $dbatoolsmod {Write-Message -Level Warning -Message "Failed to Drop database $dbName on $destination."}
                            }
                        }
                    }
                }
                ...
                if ($destserver.Databases[$dbName] -and -not $NoDrop) {
                    & $dbatoolsmod {Write-Message -Level Warning -Message "$dbName was not dropped."}
                }

                if ($CopyFile) {
                    & $dbatoolsmod {Write-Message -Level Verbose -Message "Removing copied backup file from $destination."}
                    try {
                        $removearray | Remove-Item -ErrorAction Stop
                    } catch {
                        & $dbatoolsmod {Write-Message -Level Warning -Message $_ -ErrorRecord $_ -Target $instance}
                    }
                }
            }
